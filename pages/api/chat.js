...
const FAQ = [
  ...
  { keywords: ["broker", "agent", "register as broker", "become broker", "join as"], answer: () =>
    `*Broker Registration*\n\n` +
  ...
  { keywords: ["plan", "subscription", "pricing", "how much broker", "broker plan", "listing limit"], answer: () =>
  ...
  { keywords: ["earn", "income", "commission", "broker pay", "paid", "payout"], answer: () =>
    ...
    `${BROKER_INFO.payout}` },
  { keywords: ["career", "job", "opening", "vacancy", "employment", "work for", "hire"], answer: () =>
  ...
  { keywords: ["google", "sign in", "login", "account", "sign up", "register"], answer: () =>
    `*Account Access*\n\n` +
...
export default async function handler(req, res) {
  ...
  try {
    // Fetch app data for data-driven responses
    const [
      { data: properties },
      { data: brokers },
      { data: officers },
    ...
    // ── PROPERTY SEARCH ──
    if (contains(msg, ["property", "properties", "land", "house", "home", "plot", "apartment", "property for sale", "show available", "listings", "buy"])) {
